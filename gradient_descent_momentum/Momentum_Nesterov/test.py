from gd_momentum import GradientDescentMomentum
import numpy as np
...
def visualize_2D_function(f: callable, history: list):
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title('Ordinary Gradient Descent')
    plt.legend(loc='best')
    plt.colorbar()
    plt.savefig('plots/ordinary_gd.png')
    plt.show()
...
def main():
    ...
    optimizer = GradientDescentMomentum(f, gradf, learning_rate=0.01, momentum=0.9 ,tolerance=1e-6, max_iterations=1000 , NAG=True)
